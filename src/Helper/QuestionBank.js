export const Questions = [
    {
      id: 90,
      prompt: "Your Email?",
      question_type: 4,
      required: 1,
    },
    {
      id: 91,
      prompt: "Your Age?",
      question_type: 4,
      required: 0,
    },
    {
      id: 92,
      prompt: "Your Gender?",
      question_type: 1,
      required: 1,
      choices: [
        { id: 777, choice_text: "Male" },
        { id: 778, choice_text: "Female" },
        { id: 779, choice_text: "Other" },
      ],
    },
    {
      id: 93,
      prompt: "Your preferred job location?",
      question_type: 1,
      required: 0,
      choices: [
        { id: 780, choice_text: "Work from home" },
        { id: 781, choice_text: "Work in office" },
        { id: 782, choice_text: "Work in hybrid way" },
      ],
    },
    {
      id: 94,
      prompt: "Your Salary Expectation (CTC in rupees)?",
      question_type: 2,
      required: 1,
      choices: [
        { id: 783, choice_text: "2-3 lakh" },
        { id: 784, choice_text: "4-5 lakh" },
        { id: 785, choice_text: "5-6 lakh" },
        { id: 786, choice_text: "6-7 lakh" },
      ],
    },
    {
      id: 95,
      prompt: "Your Educational Qualification?",
      question_type: 2,
      required: 0,
      choices: [
        { id: 787, choice_text: "Illiterate" },
        { id: 788, choice_text: "Below 10th standard" },
        { id: 789, choice_text: "10th passed" },
        { id: 790, choice_text: "12th passed" },
        { id: 791, choice_text: "Undergraduate" },
        { id: 792, choice_text: "Postgraduate" },
      ],
    },
    {
      id: 96,
      prompt: "Your Skills",
      question_type: 3,
      required: 1,
      choices: [
        { id: 793, choice_text: "Node.js" },
        { id: 794, choice_text: "ReactJS" },
        { id: 795, choice_text: "Mongodb" },
        { id: 796, choice_text: "Express" },
      ],
    },
    {
      id: 97,
      prompt: "Your Extra Activities",
      question_type: 3,
      required: 0,
      choices: [
        { id: 797, choice_text: "Cricket" },
        { id: 798, choice_text: "Football" },
        { id: 799, choice_text: "Tennis" },
        { id: 800, choice_text: "Badminton" },
      ],
    },
    {
      id: 98,
      prompt: "Your Photo",
      question_type: 5,
      required: 1,
    },
    {
      id: 99,
      prompt: "Your CV in pdf",
      question_type: 5,
      required: 0,
    },
  ];
  






// export const Questions =[
//     {
//         prompt: "Your Email?",
// optionA: "24",
// optionB: "21",
// optionC: "4",
// optionD: "2", 
// answer: "optionC",
//     },
//     {
//         prompt: "What is yamini?",
// optionA: "24",
// optionB: "21",
// optionC: "4",
// optionD: "2", 
// answer: "optionC",
//     },
//     {
//         prompt: "blah blah?",
// optionA: "24",
// optionB: "21",
// optionC: "4",
// optionD: "2", 
// answer: "optionC",
//     }
// ];
